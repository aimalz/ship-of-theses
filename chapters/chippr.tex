...
\chapter{Cosmological Hierarchical Inference with Probabilistic Photometric Redshifts\chaplabel{chippr}}
...
An alternative to point estimation of photo-$z$s is redshift probability distribution function (PDF) estimation, in which rather than an MLE point estimate, the full posterior (or likelihood) PDF of the redshift of a galaxy is reported \citep{Koo1999}.  
...
This situation may be considered to be a probabilistic generative model, illustrated by the directed acyclic graph of \Fig{fig:flow}.  
...
first threshold (described in \Sect{sec:acorr}) is defined in terms of 
...
\begin{figure}
	...
	\caption{This figure shows samples (colored lines) of $p(z|\vec{\theta})$ where 
		...
		defined in \Sect{sec:exp} with Eq. \ref{eq:priorcov}.}
	\label{fig:prior}
\end{figure}
...
The results of the computation described in \Sect{sec:mcmc} are evaluated 
...
Here we present the results of the informative tests of \Sect{sec:exp}, 
...
The sampler was run on the two datasets described in \Sect{sec:imprecision}.  In the case of less noisy data (Fig. 
...
according to the procedure of \Sect{sec:inaccuracy} is described here.  
...
pseudo-random sample of BOSS DR10 data described in \Sect{sec:data} are 
