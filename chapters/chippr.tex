...
\aim{Might not be distinguishing definitions, propositions, lemmas, and theorems properly, and also not using the notation right with respect to actually putting the proof where it's supposed to be rather than in the problem statement.}
...
\aim{Should I include the proof by induction that follows from Lemma \ref{lem:independence} or leave it as an exercise for the reader?}
...
\aim{Restructure this for consistency.}
\begin{theorem}
...
We may generalize the two-redshift universe to a many-redshift universe by considering the generalization of the binomial theorem to the birthday problem, which asks for the probability that $N_{z}$ people in a room of $N_{tot}$ people share the same birthday.
...
\begin{itemize}
	...
	\item \Pzpdf s are also conditioned on the prior information, $I_{P}$, used to derive them, such as a template library or training set, as well as the method used to combine this information with the data of each galaxy.
...
\aim{Section still requires text description, consistent notation.}
The simplistic notation of the above sections glosses over something important.
What differentiates \pzpdf s for each galaxy?
...
\begin{theorem}
	The mathematically valid $\langle N(z) \rangle$ asymptotically approaches the true number $N^{\dagger}$ of galaxies with redshift $z'$ under perfectly informative photometry, i.e. $\pr{z \gvn \data_{i}} = \delta(z,\ z^{\dagger}_{i})$.
\end{theorem}

\begin{proof}
	First, we replace all instances of $\pr{z_{i}}$ in \Eq{eqn:complete} with $\pr{z \gvn \data_{i}}$, yielding
	\begin{align}
	\eqlabel{eqn:proofwithdata0}
	...
	\end{align}
	For perfectly informative data, each of these $\pr{z \gvn \data_{i}}$ becomes $\delta(z,\ z^{\dagger}_{i})$, making \Eq{eqn:proofwithdata}
	\begin{align}
	...
	\end{align}
	Each of the delta function terms will be 1 when $z' = z^{\dagger}_{i_{k}}$ and 0 otherwise.
	...
	\end{align}
	Since the entire quantity in brackets is 1, we arrive at the desired $\langle N(z') \rangle = N^{\dagger}$, the same as what stacking yields.
\end{proof}

\aim{restate that this is one of the only conditions where stacking definitely works, and this is the assumption you have to make for the right thing to give the same result as stacking}
...
Though the conditions of \Sect{sec:informative} and \Sect{sec:prior} may not have been strongly violated in the past, they will be nowhere near satisfied for ongoing and upcoming surveys.
...
\aim{Add in the math and constants here.}
