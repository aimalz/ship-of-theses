...
%\aim{implicit \pzpdf\ vs implicit-weighted}
...
\begin{figure}
	\begin{center}
		...
		\caption[The directed acyclic graph of the CHIPPR model]
			{The directed acyclic graph of the CHIPPR model, where circles indicate random variables and arrows indicate causal relationships.
...
& - \ln[\pr{\data_{j} \gvn z}] - \ln[\pr{\data_{j} \gvn \ndphi^{*}}].
\end{split}
\end{align}
...
\begin{split}
\eqlabel{eqn:final}
\ln[\pr{\ndphi \gvn \{\data_{j}\}}] \propto& \ln \left[\integral{\exp \left[\sum_{j=1}^{J} \left(\ln[\pr{z \gvn \data_{j}, \ndphi^{*}}] + \ln[\pr{z \gvn \ndphi}] - \ln[\pr{z \gvn \ndphi^{*}}]\right)\right]}{z}\right]\\
& + \ln[\pr{\ndphi}],
\end{split}
\end{align}
...
The method outlined here is valid regardless of how the implicit \pz\ log-posteriors are obtained so the many approaches to producing \pzpdf s will not be rehashed; though the matter is outside the scope of this \paper, reviews of various methods have been presented in the literature \citep{sheldon_photometric_2012, ball_robust_2008, carrasco_kind_tpz:_2013, carrasco_kind_exhausting_2014}, and will be briefly reviewed in \Chap{pzdc1}.
%
%\begin{align}
%\begin{split}
%\eqlabel{eqn:fullpost}
...
These two approaches have been compared to one another in the past \citep{hildebrandt_cfhtlens:_2012, benjamin_cfhtlens_2013, asorey_galaxy_2016} but not to \Chippr.
...
The KLD is explored in detail in the Appendix to \Chap{qp}.
...
\begin{figure*}
	\begin{center}
		\includegraphics[width=0.98\textwidth]{figures/chippr/flowchart.pdf}
		\caption[A flow chart illustrating the forward model used to generate mock data in the validation of \Chippr]
		...
		Ovals indicate a variable that must be chosen in order to generate the data, rectangles indicate a methodological procedure, and rounded rectangles indicate a produced quantity.
...
%		\includegraphics[width=0.3\textwidth]{figures/chippr/bias_scatplot.png}
		\includegraphics[width=0.74\textwidth]{figures/chippr/single_lsst-mega_scatter.png}
		\caption[The joint probability space of true and estimated redshift for the three concerning \pz\ systematics at the level of the \lsst\ requirements]
		{
%			\aim{Include slice further up so we can see outliers.
%			Label panel.}
			...
			The main panel shows samples (black points) in the space of mock data and redshift, akin to the standard scatterplots of true and estimated redshift, the $z_{\mathrm{true}} = z_{\mathrm{phot}}$ diagonal (gray line), and posterior probabilities evaluated at the given estimated redshift (colored step functions).
...
\begin{figure}
	\includegraphics[width=0.49\textwidth]{figures/chippr/single_varsigmas-mega_scatter.png}
	\includegraphics[width=0.49\textwidth]{figures/chippr/thesis_hivarsig-mega_scatter.png}
	\caption[Examples of mock \pzpdf s generated with intrinsic scatter at the \lsst\ requirements and twice the \lsst\ requirements]
			{
%				\aim{Include slice further up.
%				Label panels.}
...
\begin{figure}
	\includegraphics[width=0.49\textwidth]{figures/chippr/single_varsigmas_log_estimators.png}
	\includegraphics[width=0.49\textwidth]{figures/chippr/thesis_hivarsig_log_estimators.png}
	\caption[The results of \Chippr\ and the alternative approaches on \pzpdf s with intrinsic scatter of the \lsst\ requirements and twice that]
		{
%			\aim{Label panels.
...
As was covered in \Chap{intro}, catastrophic outliers tend to be distributed non-uniformly across the space of observed and true redshift.
...
%		\aim{Enlarge legend text.
%			Use integer ticks on both axes.
%			Label panel.
%			Add ``results of inference'' watermark.
%			Show the mean redshift for each estimator of N(z), cite the DES papers that motivate this.}
...
\begin{figure}
	\includegraphics[width=0.49\textwidth]{figures/chippr/thesis_eout-mega_scatter.png}
	\includegraphics[width=0.49\textwidth]{figures/chippr/thesis_rout-mega_scatter.png}
	\caption[Examples of \pzpdf s with a catastrophic outlier population like that seen in template-fitting \pzpdf\ codes and machine learning \pzpdf\ codes]
		{
%			\aim{Include slice further up.
%			Label panels.}
...
\begin{figure}
	...
	\caption[The results of \Chippr\ and the alternative approaches on \pzpdf s with catastrophic outliers like those seen in template-fitting \pzpdf\ codes and machine learning \pzpdf\ codes]
		{
%			\aim{Label panels.
%			Show the mean redshift for each estimator of N(z), cite the DES papers that motivate this.}
...
\begin{figure}
	\includegraphics[width=0.49\textwidth]{figures/chippr/thesis_neghivarbias-mega_scatter.png}
	\includegraphics[width=0.49\textwidth]{figures/chippr/thesis_neghivarbias_log_estimators.png}
	\caption[Examples of \pzpdf s with ten times the bias of the \lsst\ requirements and the results of \Chippr\ and the alternative approaches on \pzpdf s with ten times the bias of the \lsst\ requirements]
...
\begin{figure}
	\includegraphics[width=0.49\textwidth]{figures/chippr/single_lsst_trpr-mega_scatter.png}
	\includegraphics[width=0.49\textwidth]{figures/chippr/single_lsst_tmpr-mega_scatter.png}
	\caption[Examples of mock \pzpdf s generated with a machine learning-like implicit prior and a template-fitting-like implicit prior]
		{
%			\aim{Include slice further up.
%			Recolor inset curves.
%			Fix curve normalization in insets.
%			Use integer ticks on both axes.
%			Enlarge axis labels.
%			Label panels.
%			Include ``mock data'' watermark.}
...
\begin{figure}
	\includegraphics[width=0.49\textwidth]{figures/chippr/single_lsst_trpr_log_estimators.png}
	\includegraphics[width=0.49\textwidth]{figures/chippr/single_lsst_tmpr_log_estimators.png}
	\caption[The results of \Chippr\ and the alternative approaches on \pzpdf s with an implicit prior like that of machine learning \pzpdf\ approaches and an implicit prior like that of template-fitting \pzpdf\ codes]
		{
%			\aim{Label panels.
%			Show the mean redshift for each estimator of N(z), cite the DES papers that motivate this.}
...
\begin{figure*}
	\begin{center}
		\includegraphics[width=0.24\textwidth]{figures/chippr/0single_lsst-mega_scatter.png}
		\includegraphics[width=0.24\textwidth]{figures/chippr/1single_lsst-mega_scatter.png}		\includegraphics[width=0.24\textwidth]{figures/chippr/2single_lsst-mega_scatter.png}
		\includegraphics[width=0.24\textwidth]{figures/chippr/3single_lsst-mega_scatter.png}
		\caption[The joint probability space of true and estimated redshift for an \lsst-like sample in four tomographic bins]
		{As in \Fig{fig:mega_scatter}, with a different tomographic bin in each panel and the three effects of intrinsic scatter, uniformly distributed catastrophic outliers, and bias at the levels of the \lsst\ SRD, given in Table~\ref{intro:tab:tab:lsstsrd}.
%		\aim{Combine panels to eliminate repeated insets, axis labels.
%			Include slice further up.
%			Label panels.}
}
...
\begin{figure*}
	\begin{center}
		...
		\caption[The result of propagating the estimators of \nz\ by stacking, the histogram of modes, \Chippr, and the true \nz to a subset of cosmological parameters]
...
If we naively used the \pzpdf\ catalog produced by a generic \pz\ estimation code and assumed a flat implicit prior, we would observe the contents of \Fig{fig:mischaracterized}.

\begin{figure*}
	\begin{center}
		\includegraphics[width=0.49\textwidth]{figures/chippr/single_lsst_trpr_wrong_log_estimators.png}
		\includegraphics[width=0.49\textwidth]{figures/chippr/single_lsst_tmpr_wrong_log_estimators.png}
		\caption[The results of \Chippr\ and the alternative approaches when run with an incorrectly specified implicit prior on \lsst-like data]
			...
			The data upon which each panel's results are based are provided in \Fig{fig:pzs-priors}, where the left corresponds to the sort of implicit prior anticipated of machine learning approaches and the right corresponds to an implicit prior like that of a template-fitting code.
			Here, \Chippr\ has been provided with a uniform implicit prior rather than those used to produce the mock \pzpdf s, and its performance is notably worse than when it is provided an accurate implicit prior, as in \Fig{fig:results-priors}.
%			\aim{Label panels.
%				Show the mean redshift for each estimator of N(z), cite the DES papers that motivate this.}
