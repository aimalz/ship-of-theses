...
Because the stacked estimator of the redshift distribution given in \Eq{eq:stackedwithdata} yields a single value $\hat{N}_{z}$ for $N_{z}$ rather than the probability distribution $\pr{N_{z}}$, we'll work with the expected value so we can compare apples to apples.
