...
The prevailing way to estimate \nz\ for a sample of $J$ galaxies $j$ is to normalize \Eq{eqn:stackwithdata}.
...
The forward model corresponds to first redshifts according to \Eq{eqn:pz} and then drawing data from the likelihood
\begin{equation}
\label{eqn:pzpdf}
...
\aim{Move some version of \Fig{fig:mega_scatter} here.
	Address bias as model misspecification.}
...
\end{enumerate}
\Fig{fig:flowchart} illustrates the implementation of this forward model as in this \paper.
...
When we constrain the redshift of a galaxy using its observed photometric data $\data_{j}$, we are effectively estimating a posterior $\Pr(z \mid d_{j})$.
...
The method outlined here is valid regardless of how the interim photo-$z$ log-posteriors are obtained so the many approaches to producing photo-$z$ probability distributions will not be discussed; though the matter is outside the scope of this paper, reviews of various methods have been presented in the literature \citep{Sheldon2012, Ball2008, CarrascoKind2013, CarrascoKind2014a}, and will be briefly reviewed in \Chap{pzdc1}.
...
In this study, we compare the results of \Eq{eqn:fullpost} to those of the two most common approaches to estimating \nz\ from a catalog of \pzpdf s: the stacked estimator of \Eq{eqn:stacked} and the distribution $n(z_{\mathrm{max}})$ of the redshifts at maximum posterior probability
\begin{equation}
\eqlabel{eq:mmap}
...
We compare the results of \Chippr\ to those of stacking and  on mock data in the form of catalogs of emulated \pzpdf s generated via the forward model discussed above and presented in detail in \Sect{sec:data}.
...
Furthermore, the \Chippr\ posterior distribution on the redshift distribution is itself broader for the higher intrinsic scatter case than for the \lsst\ requirements, though the marginalized maximum likelihood estimator is unaffected.
