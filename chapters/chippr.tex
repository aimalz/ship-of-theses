...
We first establish some basic definitions:
\begin{enumerate}
	\item The \textit{state space} of a particular galaxy's redshift can be broken down into two states, that of redshift equal to $z$ and that of any other redshift $\sim z$.
	\item The elements of the state space are \textit{disjoint} if one occurring means that all others do not occur; our single galaxy cannot have both redshift $z$ and a redshift $\sim z$.
	\item If the \textit{probability density} of a galaxy having a particular redshift $z$ is $p(z) = \frac{N(z)}{N_{tot}} \equiv n(z)$ of finding a galaxy at a given redshift is the number of galaxies at that redshift divided by the total number of galaxies.
	\item A probability density must satisfy the \textit{normalization} condition $\int p(z) dz = 1$, which is also $p(z) + p(\sim z) = 1$ for our single galaxy relative to a specific redshift $z$.
\end{enumerate}
These definitions pertain to the redshift of a single galaxy, but this document pertains to the redshifts of ensembles of galaxies.
\begin{enumerate}
	\item The number of galaxies at a particular redshift $N(z) = N_{z}$ is a \textit{summary statistic} of th
	\item The probability of multiple \textit{statistically independent} events occurring is the product of the probabilities of the individual events; for example, the probability of finding a galaxy $i$ at $z_{i}$ and another galaxy $j$ at $z_{j}$ is $p(z_{i} \cap z_{j}) = p(z_{i}) p(z_{j})$ if the redshift of one is not causally dependent on the redshift of the other.
	\item 
\end{enumerate}

The \textit{state space} of the redshift distribution $N(z)$ at some particular redshift $z$ encompasses all possible non-negative integers $N_{z} = 0, \dots, N_{tot}$, so $N_{tot}+1$ possibilities.
