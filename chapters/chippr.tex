...
To apply Lemma \ref{lem:expectednz}, we next seek $\pr{N_{z}}$.

\aim{Break this up into some smaller pieces with a lemma or two.}
\begin{theorem}\label{thm:general}
	The expected value of $N_{z}$ is given by
	\begin{align}
	\eqlabel{eq:complete}
...
\end{align}
	\textul{Proof}:\\
	Let's start from a single combination $\mathbb{S}^{z}_{c}$.
	...
	Invoking Definition \ref{def:disjoint}, we know that $\pr{N_{z}} = \sum_{c}^{C} \pr{\mathbb{S}^{z}_{c}}$.
	Then we simply apply Definition \ref{def:expected} to arrive at \Eq{eq:complete}.
\end{theorem}

The goal here is to figure out when, if ever, this is equivalent to \Eq{eq:stacked}.
...
\begin{equation}
\eqlabel{eq:binomial}
p(N_{z} \gvn \phi_{b}) = \binom{N_{tot}}{N_{z}} \times \phi_{b}^{N_{z}} \times (1 - \phi_{b})^{N_{tot} - N_{z}} .
\end{equation}
\end{theorem}
