...
Upcoming wide-field imaging surveys such as the Legacy Survey of Space and Time 
...
for each galaxy in the survey \citep{tanaka_photometric_2018, de_jong_third_2017, sheldon_photometric_2012}.
...
\citep[Section 4.2.2]{juric_lsst_2017}, with plans to store \pz s derived by 
multiple methods.
...
In \Sect{sec:methods}, we outline how \qp\ can be used to optimize the 
choice of \pz\ parametrization.
In \Sect{sec:data}, we describe the mock datasets on which we 
demonstrate such an analysis.
We present the results of this procedure in \Sect{sec:results} and make 
recommendations for the use of \qp\ by the photo-$z$ community in 
\Sect{sec:conclusions}.
...
A subset of the included metrics are described in \Sect{sec:metric}.
...
(\Sect{sec:bins}), random samples (\Sect{sec:samples}), and 
regular quantiles (\Sect{sec:quantiles}).
...
\begin{figure}
	\begin{center}
		...
		\caption[\qp\ approximation of a continuous 1-dimensional PDF]
			{\qp\ approximation of a continuous 1-dimensional PDF (thick, solid 
...
subsampling \citep{hoyle_dark_2018}.
...
\citet{polsterer_uncertain_2016, schmidt_evaluation_2020})
...
In Appendix~\ref{app:kld}, we review the properties of the KLD and provide some intuition 
for it.
...
\citep[See][for other work comparing the 
accuracy of \pz s produced by different methods.]{schmidt_evaluation_2020, tanaka_photometric_2018, 
...
\begin{figure*}
	\begin{center}
		...
		\caption[Example \pz s from the two mock LSST datasets]
...
sample, the Buzzard-highres-v1.0 mock galaxy catalog of \citet{derose_buzzard_2019} in 
...
\citep{lsst_science_collaboration_lsst_2009}, that will have $S/N \gtrsim 30$ in multiple bands.
...
We evaluate the metrics of \Sect{sec:metric} on 10 random instantiations 
of catalogs of $N_{g}=100$ galaxies drawn randomly from each of the datasets 
discussed in \Sect{sec:data} and with each of $N_{f}=3,\ 10,\ 30,\ 100$ 
stored parameters for the three formats of \Sect{sec:approx}.
...
\begin{figure}
	\begin{center}
		...
		\mgdata dataset]
			{The distribution of log-KLD values for $N_{g}=100$ \pz s from the 
...
\begin{figure*}
	\begin{center}
		...
		\caption[The means of the mean, variance, and skewness of the log-KLD distributions for each data set as a function of the number $N_{f}$ of stored parameters for each storage format]
...
\begin{figure*}
	\begin{center}
		...
		\caption[The median $\log_{10}$-percent errors on the mean, variance, and skewness of the \pz s for each dataset as a function of the number $N_{f}$ of stored parameters per \pz\ for each storage format]
...
\begin{figure}
	\begin{center}
		...
		for a subsample of galaxies drawn from the \ssdata data mock 
		catalog]
			{An example of the stacked estimator of the redshift distribution, 
...
\begin{figure*}
	\begin{center}
		...
		s, as a function of number $N_{f}$ of stored parameters, for each storage format]
			{The KLD between $\hat{n}(z)$ derived from approximate \pz\ 
			representations and $\hat{n}(z)$ derived from the original, high-resolution \pz 
...
\begin{figure*}
	\begin{center}
		...
		\caption[The percent error on the mean, variance, and 
		skewness of the stacked estimator $\hat{n}(z)$ of the redshift 
		distribution for each dataset as a function of the number $N_{f}$ of stored 
		parameters for each storage format]
