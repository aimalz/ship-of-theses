...
%and whose public draft\footnote{\url{{https://github.com/LSSTDESC/PZDC1paper}}} is currently in \desc\ internal review.
...
%\aim{TODO: propagate to paper on GH}
...
%\aim{TODO: propagate to paper on GH}
In currently available spectroscopic training sets, systematic incompleteness and incorrectly assigned redshifts are a harrowing problem, particularly at faint magnitudes.  
...
Characterizing and mitigating these problems will be a major undertaking which we will cover in future papers, as we discuss briefly in \Sect{sec:futureexperiments}.  
...
In the absence of simulated data drawn from known redshift distributions, the very concept of a ``true PDF'' for an individual galaxy is unavailable, and we must instead rely on measures of ensemble behaviour to characterize PDF quality (see \Sect{sec:metrics} for further discussion).
...
%mjrosenb: removed in pxdc1 paper
...
Here we summarize the twelve \pzpdf\ codes compared in this study, listed in \Tab{tab:list_of_codes}, which include both established and emerging approaches in template fitting and machine learning.
...
% and how to treat non-detections in one or more bands;
%some codes ignore incomplete bands, while others replace the value with either an estimate for the detection limit, the mean of other values in the training set, or another default value.
...
\begin{table*}  %%% DATA TABLE %%%
	\caption[List of \pzpdf\ codes considered in \Chap{pzdc1}]
	{List of \pzpdf\ codes featured in this study}
	\tablabel{tab:list_of_codes}\resizebox{\textwidth}{!}{
		\begin{tabular}{lll}
			\hline
			...
			\eazy~\citep{brammer_eazy:_2008}		   & template fitting & \url{https://github.com/gbrammer/eazy-photoz} \\
			\lephare~\citep{arnouts_measuring_1999}	   & template fitting	& \url{http://www.cfht.hawaii.edu/~arnouts/lephare.html} \\
			\annz~\citep{sadeh_annz2:_2016}		     & machine learning	& \url{https://github.com/IftachSadeh/ANNZ} \\
			\cmnn~\citep{graham_photometric_2018}        & machine learning & \url{https://github.com/dirac-institute/CMNN_Photoz_Estimator} \\
			\delight~\citep{leistedt_data-driven_2017}   & hybrid           & \url{https://github.com/ixkael/Delight} \\
			\flexzboost~\citep{izbicki_converting_2017} & machine learning & \url{https://github.com/tpospisi/flexcode}; \url{https://github.com/rizbicki/FlexCoDE}\\
			%\textsc{Frankenz} 	& ML 	& \citet{Speagle:frankenz}	& \url{https://github.com/joshspeagle/frankenz} \\
			\gpz~\citep{almosallam_gpz:_2016}	   & machine learning	& \url{https://github.com/OxfordML/GPz} \\
			\metaphor~\citep{cavuoti_metaphor:_2017}   & machine learning	& \url{http://dame.dsf.unina.it}\\

			\skynet~\citep{graff_skynet:_2014}       & machine learning & \url{http://ccpforge.cse.rl.ac.uk/gf/project/skynet/} \\
...
Easy and Accurate Photometric Redshifts from Yale \citep[\eazy\footnote{\url{https://github.com/gbrammer/eazy-photoz}},][]{brammer_eazy:_2008} extends the basic \chisq\ fit procedure that defines template-fitting approaches.
...
Photometric Analysis for Redshift Estimate \citep[\lephare,][]{arnouts_measuring_1999,ilbert_accurate_2006} uses the $\chi^2$ of \Eq{eq_temp_chi} to match observed colours with those predicted from a template set.
...
\annz \footnote{\url{https://github.com/IftachSadeh/ANNZ}} \citep{sadeh_annz2:_2016} supports several machine learning algorithms, including artificial neural networks (ANN), boosted decision tree, and k-nearest neighbour (KNN) regression.
...
In this paper, we used \texttt{xgboost} \citep{chen_xgboost:_2016} for the regression; it should, however, be noted that \texttt{FlexCode-RF}\footref{flexzboost_github}, based on Random Forests, generally performs better on smaller datasets.
...
%, already validated on photo-$z$'s in several cases \citep{de_jong_third_2017,Cavuoti:17b,Cavuoti:15,Brescia:14,Brescia:13,Biviano:13}.
...
\skynet \footnote{\url{http://ccpforge.cse.rl.ac.uk/gf/project/skynet/}} \citep{graff_skynet:_2014} employs a neural network based on a second-order conjugate gradient optimization scheme \citep[see][for further details]{graff_skynet:_2014}. %It has been used efficiently for redshift PDF estimates \citep{sanchez_photometric_2014,Bonnett:15,Bonnett:16}.
...
Galaxies have redshifts $z$ and photometric data $d$ drawn from a joint probability space $p(z, d)$ in nature, and each observed galaxy $i$ has a \textit{true posterior \pzpdf}\ $p(z \vert d_{i})$.% as well as a true likelihood $p(d \vert z_{i})$.
...
%\aim{TODO: add a footnote explaining the two problems: sparsity and forward modeling}
...
(See \Sect{sec:futureexperiments} for a discussion of how one might circumvent this limitation.)
...
% namely the often implicit assumptions $I_{H}$ necessary for the method to be valid and any inputs $I_{D}$ it takes as prior information, such as a template library or training set.
...
The PIT distribution has been used to quantify the performance of \pzpdf\ methods in the past \citep[e.~g.~][]{bordoloi_photo-z_2010,polsterer_uncertain_2016,tanaka_photometric_2018}.
...
% an approximating univariate distribution $\hat{f}(z)$ and a reference distribution $\tilde{f}(z)$.
...
(See \Sect{sec:flexzboost} for a review of the notation.)
...
\end{equation}
% \aim{edit for clarity: no "covariates", actually need to understand how we got this}
...
% \aim{Must understand this for defense!!! notation -- check script on what we actually did}
...
The most striking differences between codes are the small-scale features induced by the interaction between the shared piecewise constant parameterization of $200$ bins for $0 < z < 2$ of \Sect{sec:metrics} and the smoothing conditions or lack thereof in each algorithm.
...
%The gray shaded band marks the $2\sigma$ variance in PIT values found using the \trainz\ algorithm with a bootstrap resampling of the training set and a sample size of 44,000 galaxies, representing a very conservative estimate of the expected errors due to the training sample size \citep{newman_spectroscopic_2015}, and thus an approximate minimal error significance compared to ideal performance.
...
\begin{table}
	...
	\centering
	\caption[The catastrophic outlier rate for the codes of \Chap{pzdc1}]
...
Close inspection of the extremes at PIT values of 0 and 1 reveal spikes in the first and last bin of the PIT histogram for some codes in \Fig{fig:pitqq}, corresponding to catastrophic outliers where the true redshift lies outside of the support of the $p(z)$.
The catastrophic outlier rates are provided in \Tab{tab:pitoutlier}.
...
\Fig{fig:pit_stats} highlights the relative values of the KS, CvM, and AD test statistics calculated by comparing the PIT distribution and a uniform distribution $U(0, 1)$, highlighting the relative rather than absolute numbers.
...
\begin{table}  %%% DATA TABLE %%%
	\centering
	\caption[CDE loss statistic of the individual \pzpdf s for each code considered in \Chap{pzdc1}]
...
(The CDE Loss metric of \Sect{sec:CDElossmetric} is an exception to this rule.)%mjrosenb: make sure this references the correct section. it looks like names have changed
...
% mrjosenb: this sentence looks like it was removed
...
\Sect{sec:metrics} discussed the difficulty in evaluating PDF accuracy for individual objects with known $(z, d)$ information but without a known $p(z, d)$.
...
% In addition to these improvements, the \lsstdesc \Pz\ Working Group plans to look at all potential methods to combine the results from multiple \pzpdf\ codes to improve accuracy, similar to the work presented in \citet{dahlen_critical_2013,carrasco_kind_exhausting_2014,Duncan:18}.
...
%	\caption[The smoothed stacked estimator $\hat{N}(z)$ of the redshift distribution produced by each code]
...
%The horizontal lines show the the result of a bootstrap resampling of the training set using 30 000 samples for \trainz, representing a conservative statistical error estimate assuming our modest-sized representative training set of galaxies, as mentioned in \Sect{sec:pitqq}.
...
%	\caption[The Kolmogorov-Smirnoff, Cramer-von Mises, and Anderson-Darling statistics for the $\hat{N}(z)$ distributions]
...
%
%\Fig{fig:pz_pointestimates} shows the $z_{PEAK}$ and $z_{WEIGHT}$ point estimates versus true redshift for all codes.
...
%\begin{figure*}
%	\centering
%	\includegraphics[width=0.49\textwidth]{figures/pzdc1/ZPEAK_AIM.png}
%	\includegraphics[width=0.49\textwidth]{figures/pzdc1/ZWEIGHT_AIM.png}
%	\caption[The density of \pz\ point estimates reduced from the \pzpdf s via the mode and main-peak-mean]
%		{The density of \pz\ point estimates (blue contours) reduced from the \pzpdf s with outliers (orange) beyond the outlier cutoff (green dashed lines), via the mode ($z_{PEAK}$, left panel) and main-peak-mean ($z_{WEIGHT}$, right panel).
%		The \trainz\ estimator (lower right sub-panels) has a shared $z_{PEAK}$ and $z_{WEIGHT}$ for the entire test set galaxy sample.
%%		\aim{TODO: Split this into two plots!}
...
%The features perpendicular to the $z_{phot} = z_{true}$ line are due to the $4000 \rm{\AA}$ break passing through the gaps between adjacent filters, most notably at $z \sim 0.4$ between the $g$ and $r$ filters and at $z \sim 1.4$ between the $z$ and $y$ filters.  
...
%Thus even the strongest codes feature populations far from the $z_{phot} = z_{true}$ line representing a genuine degeneracy in the space of colours and redshifts.
...
%\begin{table*}
%	\begin{center}
%		\caption[\Pz\ point estimate summary statistics]
