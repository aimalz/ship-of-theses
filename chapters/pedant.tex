...
This \paper\ concerns the method by which one obtains the distribution of galaxy redshifts from catalogs of \pzpdf s by addressing why established methodologies appear correct despite being invalid, work inspired by conversations with Phil Marshall and Daniel Gruen during the summer of 2017 that was refined by discussions with Boris Leistedt in spring 2018.
...
Stacking is bolstered by pervasive misunderstandings about the causal structure of the problem and \pzpdf s as probabilistic objects overall \citep{jarvis_rmjarvis_2018, malz_aimalz_2018}.
...
\begin{definition}\label{def:pdens}
	...
	I'll need to introduce the function $f_{z}(\ndphi)$ whose integral about $z'$ is understood as $\pr{z'}$, currently not mentioned until Equation~\ref{chippr:eq:eqn:fz}.}
\end{definition}
