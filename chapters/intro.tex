...
Calculations of correlation functions of cosmic shears and galaxy positions used to constrain the cosmological parameters require large numbers of high-confidence redshifts of surveyed galaxies.  
...
If the true redshifts $\{z_{i'}\}$ were known, the redshift probabilities would be delta functions $\{\delta(z, z'_{i})\}$ centered at the true redshift, and the redshift distribution could be approximated by a histogram or other interpolation of the delta functions $\{\delta(z, z'_{i})\}$.
When \pzpdf s are available instead of true redshifts, the simplest approach reduces them to point estimates of redshift by using $\delta(z, \hat{z}_{i})$ in place of $\delta(z, z'_{i})$, where $\hat{z}_{i}$ may be the mode (maximum) of the \pzpdf, though there are other, more principled options \citep{tanaka_photometric_2018}.
