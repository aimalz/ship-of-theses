...
The projection of these effects into the space of true and estimated redshifts is discussed extensively in \Chap{pzdc1}, and their impact on cosmological parameter constraints is addressed in \Chap{chippr}, but some introductory material will motivate the following discussion.
...
There are several types of non-Gaussian deviation from a perfect correspondence between redshift and data in \Fig{fig:pedagogical_scatter}.
There is scatter about the diagonal due to the coarseness of the photometric filters, with larger scatter perpendicular to the diagonal where highly identifiable spectral features pass between the filters, as well as higher scatter at high redshifts due to galaxies being dimmer.
There are populations of outliers, far from the diagonal, indicating galaxies for which the redshift estimate is very far from the true redshift, showing that outliers are not uniformly distributed.
...
Based on the goals of a photometric galaxy survey, requirements can be placed on the tolerance to these effects.
For example, the Large Synoptic Survey Telescope (\lsst) states requirements for the main cosmological sample in the \lsst\ Science Requirements Document (SRD)\footnote{\url{https://docushare.lsstcorp.org/docushare/dsweb/Get/LPM-17}} \citep{the_lsst_dark_energy_science_collaboration_lsst_2018}, reproduced in \Tab{tab:lsstsrd}.
...
An example of this selection bias in the context of \project{zCOSMOS} \citep{lilly_zcosmos_2009} and \project{COSMOS} \citep{laigle_cosmos2015_2016} is shown in \Fig{fig:selection-bias}, which shows a distinct lack of support between the \project{COSMOS} and \project{zCOSMOS} galaxy populations in magnitudes and photometric redshift estimate.
...
As other systematics have been resolved, the uncertainties associated with \pz s have come to dominate the uncertainties on estimates of cosmological parameters made by current surveys such as \des \citep{hoyle_dark_2017}, \project{HSC} \citep{tanaka_photometric_2018}, and \project{KiDS} \citep{hildebrandt_kids-450:_2017}.
...
In terms of \Fig{fig:pedagogical scatter}, \pzpdf s are horizontal cuts, probabilities of redshift conditioned on a specific value of data, i.e. posteriors, which constrain redshifts, whereas vertical cuts through this space are probabilities of data conditioned on a specific redshift, i.e. likelihoods, from which data is actually drawn.
...
\aim{Provide equations/citations for the use of \Nz\ in cosmology from \citet{mandelbaum_precision_2008, sheldon_photometric_2012, bonnett_redshift_2016}.}

If the true redshifts $\{z_{i}^{\dagger}\}$ were known, the redshift probabilities would be delta functions $\{\delta(z, z_{i}^{\dagger})\}$ centered at the true redshift, and the redshift distribution could be approximated by a histogram or other interpolation of the delta functions $\{\delta(z, z_{i}^{\dagger})\}$.
When \pzpdf s are available instead of true redshifts, the simplest approach reduces them to point estimates of redshift by using $\delta(z, \hat{z}_{i})$ in place of $\delta(z, z_{i}^{\dagger})$, where $\hat{z}_{i}$ may be the mode (maximum) of the \pzpdf, though there are other, more principled options \citep{tanaka_photometric_2018}.
...
I go on to present one such an alternative to stacking, the Cosmological Hierarchical Inference with Probabilistic Photometric Redshifts (\Chippr) model, in \Chap{chippr}.
\Chippr\ is a probabilistic graphical model (PGM) that encapsulates the causal relationships between the redshift distribution, samples of individual galaxy redshifts, and photometric data of individual galaxies.
I also present the \chippr\footnote{https://github.com/aimalz/chippr} code that implements the \Chippr\ model and includes a comprehensive suite of tools for testing arbitrarily realistic mock \pzpdf\ catalogs.
Using \chippr\ and the \cosmolike\ forecasting suite, I show that stacking causes a substantially higher information loss, corresponding to broader 1-$\sigma$ error ellipses on the cosmological parameters, whereas \chippr is unbiased if the method used to obtain the \pzpdf s is well-understood.
...
Of course, this brief review does not cover all ways to obtain \pzpdf s; many more may be found in the literature, along with comparisons thereof \citep{Hildebrandt2010, Dahlen2013, Sanchez2013, Bonnett2015, tanaka_photometric_2018}.
Some current work aims to vet \pzpdf\ generation methods \citep{Wittman2016, polsterer_dealing_2016}.
\Chap{pzdc1} details the comparative study of Schmidt, Malz, and Soo, et al. (\aim{Figure out how to cite this as in review with a link to the public draft?}) but goes beyond the scope of previous attempts to identify the best \pzpdf\ technique by critiquing the evaluation criteria themselves from a mathematically principled perspective.
...
In the absence of an obvious best choice for the method used to derive \pzpdf s, \lsst\ has planned to provide several \pzpdf s per galaxy obtained by a variety of methods rather than choosing one, effectively hedging bets on which may ultimately prevail.
...
\Chap{pedant} isolates the conditions under which stacking is actually valid, showing that the necessary assumptions were to a limited extent true for early galaxy surveys but will not be upheld whatsoever by future surveys, motivating the migration to new methods.
\Chap{chippr} presents one such method, a mathematically rigorous methodology for inferring \Nz\ from a catalog of \pzpdf s, including the effect of incorrectly inferred \Nz\ on cosmological parameter estimates.
...
{\item For \chap{pzdc1}, I led the choice, evaluation, and interpretation of the comparison metrics, and I devised and implemented the \trainz\ technique of \pzpdf\ estimation.
